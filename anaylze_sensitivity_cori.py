...
def test_sensitivity_regions(files_loc,my_model,nregions):
    ...
    for i in range(len(param_names)):
        ...
        for fn in param_files:
            if os.stat(fn).st_size > 0 and not 'params' in fn:
                print(f'opening fn - {fn}')
                with open(fn, 'rb') as f:
                    curr_volts = pkl.load(f)
                    #print(curr_volts)
...
def main():
    ...
    try:
        ...
        print('no short name')
        short_name = None    
    files_loc = '/global/cscratch1/sd/roybens/Sensitivity_raw/' + m_type + '_' + e_type + '/'
...
def main_regions():
    ...
    try:
        ...
        print('no short name')
        short_name = None    
    files_loc = '/global/cscratch1/sd/zladd/Sensitivity_raw_regions/' + m_type + '_' + e_type + '/'
    #files_loc = './'
    ...
    with open(os.path.join(files_loc, 'ecds.pkl'),'wb') as f:
        pickle.dump(ECDS, f)
...
def main_regions():
    ...
    e_type = sys.argv[2]
    cell_id = sys.argv[3]
    files_loc = '/global/cscratch1/sd/zladd/Sensitivity_raw_regions/' + m_type + '_' + e_type + '_' + str(cell_id) +'/'
    #files_loc = './'
    if (len(os.listdir(files_loc))<4):
        print(f'{files_loc} has less than 4 files')
        with open("to_analyze.txt", "a") as myfile:
